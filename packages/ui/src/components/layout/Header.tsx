...
import { HeaderIconActionButton } from "./header/HeaderIconActionButton";
import {
...
export const Header: React.FC<HeaderProps> = ({
	...
	const toggleBottomTerminal = useUIStore(
		(state) => state.toggleBottomTerminal,
	);
	...
	const contextPanelByDirectory = useUIStore(
		(state) => state.contextPanelByDirectory,
	);
	...
	const [isDevShutdownInFlight, setIsDevShutdownInFlight] =
	...
	const [isMobileRateLimitsOpen, setIsMobileRateLimitsOpen] =
		React.useState(false);
	const [isDesktopServicesOpen, setIsDesktopServicesOpen] =
		React.useState(false);
	...
	const [currentInstanceIsLocal, setCurrentInstanceIsLocal] =
		React.useState(true);
	const [remoteUpdateDialogOpen, setRemoteUpdateDialogOpen] =
		React.useState(false);
	const [remoteUpdateInfo, setRemoteUpdateInfo] =
		React.useState<UpdateInfo | null>(null);
	const [remoteUpdateChecking, setRemoteUpdateChecking] = React.useState(false);
	const [remoteUpdateError, setRemoteUpdateError] = React.useState<
		string | null
	>(null);
	...
	const checkRemoteInstanceUpdate = React.useCallback(async () => {
		...
		try {
			...
			const response = await runtimeFetch(
				`/api/openchamber/update-check?${params.toString()}`,
				{
					method: "GET",
			...
			setRemoteUpdateInfo({
				...
				nextSuggestedCheckInSec:
					typeof data.nextSuggestedCheckInSec === "number"
			...
			setRemoteUpdateError(
				error instanceof Error
					? error.message
					: t("header.services.remoteUpdate.error"),
			);
		} finally {
	...
	const currentSessionLive = React.useMemo(() => {
		...
		return (
			liveSessions.find((s) => s.id === currentSessionId) ??
	...
	const shortcutLabel = React.useCallback(
		(actionId: string) => {
	...
	const handleDevShutdown = React.useCallback(async () => {
		...
		try {
			try {
				for (const [, dirState] of useTerminalStore
					.getState()
					.sessions.entries()) {
			...
			try {
				...
				if (devRes.ok) {
					...
					const shutdownRes = await runtimeFetch("/api/system/shutdown", {
						method: "POST",
					});
	...
	useEffect(() => {
		const handleKeyDown = (e: KeyboardEvent) => {
			const toggleServicesCombo = getEffectiveShortcutCombo(
			...
			const cycleServicesCombo = getEffectiveShortcutCombo(
				"cycle_services_tab",
				shortcutOverrides,
			);
			if (eventMatchesShortcut(e, cycleServicesCombo)) {
				...
				const nextIndex =
					currentIndex === -1 ? 0 : (currentIndex + 1) % tabValues.length;
		...
		<>
			{showPlanTab && (
				<Tooltip>
					<TooltipTrigger asChild>
						<button
							...
							className={cn(
								...
								isContextPlanActive && "bg-[var(--interactive-hover)]",
							)}
						>
					...
					<TooltipContent>
						<p>
							{t("header.actions.planWithShortcut", {
								shortcut: shortcutLabel("toggle_context_plan"),
							})}
						</p>
					</TooltipContent>
				</Tooltip>
			...
			<HeaderIconActionButton
				title={t("header.actions.terminalPanelWithShortcut", {
					shortcut: shortcutLabel("toggle_terminal"),
				})}
			...
			<HeaderIconActionButton
				title={t("header.actions.rightSidebarWithShortcut", {
					shortcut: shortcutLabel("toggle_right_sidebar"),
				})}
					...
					<HeaderIconActionButton
						...
						title={
							isNewSessionDraftOpen
								...
								: t("header.actions.openSessionMiniChat")
						}
						ariaLabel={
							isNewSessionDraftOpen
								? t("header.actions.newMiniChatAria")
								: t("header.actions.openSessionMiniChatAria")
						}
						onClick={handleOpenCurrentMiniChat}
						className={cn(
							desktopHeaderIconButtonClass,
							showDesktopHeaderContextUsage ? "mr-3.5" : "mr-1",
						)}
