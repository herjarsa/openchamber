...
export type BuildGroupedSessionsArgs = Args & {
...
export const buildGroupedSessions = (
  ...
  const buildProjectNode = (session: Session): SessionNode => {
    // When the user has hidden subagent sessions, every parent renders with
    // an empty children list. Subagent sessions are still excluded from the
    // top-level "roots" list above, so this is the only branch that needs
    // toggling.
...
return archivedKey;
  ...
  const groups: SessionGroup[] = [{
    ...
    label: (projectIsRepo && projectRootBranch && projectRootBranch !== 'HEAD')
      ? args.t('sessions.sidebar.grouping.projectRootWithBranch', { branch: projectRootBranch })
      : args.t('sessions.sidebar.grouping.projectRoot'),
  ...
  if (archivedSessions.length > 0) {
    groups.push({
      id: 'archived',
      label: args.t('sessions.sidebar.grouping.archived'),
      branch: null,
      description: args.t('sessions.sidebar.grouping.archivedDescription'),
      isMain: false,
...
export const useSessionGrouping = (args: Args) => {
  ...
  return {
    ...
    buildGroupedSessions: buildGroupedSessionsHook,
  };
};
