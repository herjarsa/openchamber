...
import {
	...
	EMPTY_QUEUE,
	FILE_MENTION_TOKEN,
	MAX_VISIBLE_TEXTAREA_LINES,
	PASTE_LINK_URL_PATTERN,
...
import { RevertedMessageDock } from "./chat-input/RevertedMessageDock";
import {
	appendInlineText,
...
const ChatInputComponent: React.FC<ChatInputProps> = ({
	...
	const confirmedMentionsRef = React.useRef<Set<string>>(
		loadConfirmedMentions(initialSessionIdRef.current),
	);
	...
	const isConfirmedFilePath = (text: string): boolean =>
		...
		text.includes("\\") ||
		text.includes(".") ||
		confirmedMentionsRef.current.has(text);
	const [inputMode, setInputMode] = React.useState<"normal" | "shell">(
		"normal",
	);
	...
	const [showCommandAutocomplete, setShowCommandAutocomplete] =
		React.useState(false);
	const [commandQuery, setCommandQuery] = React.useState("");
	const [showSkillAutocomplete, setShowSkillAutocomplete] =
		React.useState(false);
	const [skillQuery, setSkillQuery] = React.useState("");
	const [showSnippetAutocomplete, setShowSnippetAutocomplete] =
		React.useState(false);
	...
	const suppressNextFileDropTextInsertTimeoutRef = React.useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	...
	const draftPersistTimerRef = React.useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	...
	const pendingPastedAttachmentFilenamesRef = React.useRef<Set<string>>(
		new Set(),
	);
	...
	const sendMessage = React.useRef((...args: any[]) =>
		Promise.resolve(
			(
				useSessionUIStore.getState().sendMessage as (...a: unknown[]) => unknown
			)(...args),
		),
	).current;
	...
	const setNewSessionDraftTarget = useSessionUIStore(
		(s) => s.setNewSessionDraftTarget,
	);
	const availableWorktreesByProject = useSessionUIStore(
		(s) => s.availableWorktreesByProject,
	);
	...
	const saveSessionAgentSelection = useSelectionStore(
		(s) => s.saveSessionAgentSelection,
	);
	const consumePendingInputText = useInputStore(
		(s) => s.consumePendingInputText,
	);
	const pendingPresetSubmit = useInputStore((s) => s.pendingPresetSubmit);
	const setPendingInputText = useInputStore((s) => s.setPendingInputText);
	const pendingInputText = useInputStore((s) => s.pendingInputText);
	const consumePendingSyntheticParts = useInputStore(
		(s) => s.consumePendingSyntheticParts,
	);
	const acknowledgeSessionAbort = useSessionUIStore(
		(s) => s.acknowledgeSessionAbort,
	);
	...
	const setActiveProjectIdOnly = useProjectsStore(
		(state) => state.setActiveProjectIdOnly,
	);
	...
	const setTimelineDialogOpen = useUIStore(
		(state) => state.setTimelineDialogOpen,
	);
	const { git: runtimeGit, vscode: vscodeApi } = useRuntimeAPIs();
	const cycleAgentShortcutOverride = useUIStore(
		(state) => state.shortcutOverrides.cycle_agent,
	);
		...
		() =>
			getEffectiveShortcutCombo(
				...
				cycleAgentShortcutOverride
					? { cycle_agent: cycleAgentShortcutOverride }
					: undefined,
			),
		[cycleAgentShortcutOverride],
	);
	...
	const currentGitStatus = useGitStore((state) =>
		currentDirectory
			? (state.directories.get(currentDirectory)?.status ?? null)
			: null,
	);
	const [showAbortStatus, setShowAbortStatus] = React.useState(false);
	const setSessionAutoAccept = usePermissionStore(
		(state) => state.setSessionAutoAccept,
	);
	const composerHighlightRef = React.useRef<HTMLDivElement | null>(null);
	const [isNarrowComposer, setIsNarrowComposer] = React.useState(false);
	const [attachmentPreview, setAttachmentPreview] =
		React.useState<ToolPopupContent>({
			open: false,
			title: "",
			content: "",
		});

	const handleShowAttachmentPreview = React.useCallback(
	...
	const handleAttachmentPreviewOpenChange = React.useCallback(
		(open: boolean) => {
	...
	const composerSnippetRanges = React.useMemo<HighlightRange[]>(() => {
		...
		while ((match = snippetRegex.exec(message)) !== null) {
			...
			ranges.push({
				start: hashStart,
				end: hashStart + 1 + trigger.length,
				style: "mentionSnippet",
			});
		}
		return ranges;
	...
	const sanitizeAttachmentsForSend = React.useCallback(
		(files: AttachedFile[] | undefined): AttachedFile[] =>
			(files ?? []).map((file) => ({
				...file,
				dataUrl:
					file.source === "server" && file.serverPath
	...
	const extractInlineFileMentions = React.useCallback(
		(
			...
			const root = (chatSearchDirectory || clientDirectory)
				.replace(/\\/g, "/")
				.replace(/\/+$/, "");
			...
			while ((match = mentionRegex.exec(rawText)) !== null) {
				...
				const normalizedMentionPath = mentionPath
					.replace(/\\/g, "/")
					.replace(/^\.\//, "")
					.replace(/^\/+/, "");
				...
				const filename =
					normalizedMentionPath.split("/").filter(Boolean).pop() ||
	...
	const [autocompleteOverlayPosition, setAutocompleteOverlayPosition] =
		React.useState<AutocompleteOverlayPosition | null>(null);
	const abortTimeoutRef = React.useRef<ReturnType<typeof setTimeout> | null>(
		null,
	);
	...
	const draftCount = useInlineCommentDraftStore(
		React.useCallback(
			(state) => {
				const sessionKey =
					currentSessionId ?? (newSessionDraftOpen ? "draft" : "");
		...
		React.useCallback(
			(state) => {
				...
				for (const draft of drafts) {
					...
					else if (draft.source === "preview-annotation")
						previewAnnotation += 1;
					else review += 1;
				}
	...
	const consumeDrafts = useInlineCommentDraftStore(
		(state) => state.consumeDrafts,
	);
	const removeInlineCommentDraft = useInlineCommentDraftStore(
		(state) => state.removeDraft,
	);
	const hasDrafts = draftCount > 0;
	const [previewConsoleCount, previewAnnotationCount, reviewCount] =
		draftSourceKey.split(":").map((entry) => Number(entry) || 0);
	const removePreviewDrafts = React.useCallback(
		(source: "preview-console" | "preview-annotation") => {
			...
			const drafts =
				useInlineCommentDraftStore.getState().drafts[sessionKey] ?? [];
			for (const draft of drafts) {
	...
	const handleOpenMobilePanel = React.useCallback(
		(panel: MobileControlsPanel) => {
			if (!isMobile) {
				return;
			}
	...
	const hasContent =
		message.trim().length > 0 || sendableAttachedFiles.length > 0 || hasDrafts;
	...
	const handleQueueMessage = React.useCallback(() => {
		...
		const attachmentsToQueue = sanitizeAttachmentsForSend(
	...
	const handleSubmit = async (options?: SubmitOptions) => {
		...
		const addMentionedSkills = (text: string) => {
			for (const name of collectInlineSkillMentions(
				text,
				availableSkillNames,
			)) {
		...
		for (let i = 0; i < queuedMessagesToSend.length; i++) {
			...
			const { sanitizedText, mention } = parseAgentMentions(
				queuedMsg.content,
				agents,
			);
			const { sanitizedText: queuedText, attachments: mentionAttachments } =
			...
			if (i === 0) {
				...
				const queuedAttachments = sanitizeAttachmentsForSend(
		...
		if (!queuedOnly && inputSnapshot.hasContent) {
			...
			const { sanitizedText: messageText, attachments: mentionAttachments } =
				extractInlineFileMentions(sanitizedText);
			const attachmentsToSend = sanitizeAttachmentsForSend(
				sendableAttachedFiles,
			);
		...
		const sessionKey =
			currentSessionId ?? (newSessionDraftOpen ? "draft" : null);
		...
		const skillMentionInstruction =
			buildSkillMentionInstruction(mentionedSkillNames);
			...
			if (commandName === "undo" && currentSessionId) {
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.compactFailed"),
					);
				}
				return;
				...
				try {
					...
					const topicBlock = topic
						...
						: "";
					const visibleText = await renderMagicPrompt(
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.summaryFailed"),
					);
				}
				return;
			} else if (
				commandName === "workspace-review" &&
				(currentSessionId || newSessionDraftOpen)
			) {
				try {
					await sessionActions.waitForConnectionOrThrow();
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.reviewFailed"),
					);
				}
				return;
			} else if (
				commandName === "handoff-review" &&
				currentSessionId &&
				!isMobile &&
				!isVSCodeRuntime()
			) {
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.planFeatureFailed"),
					);
				}
				return;
			} else if (
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.catchUpFailed"),
					);
				}
				return;
			} else if (
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.debugFailed"),
					);
				}
				return;
			} else if (
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.weighFailed"),
					);
				}
				return;
			} else if (
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.exploreFailed"),
					);
				}
				return;
			}
		...
		try {
			...
			console.warn(
				"[ChatInput] Failed to expand snippets, sending original text:",
				error,
			);
		}
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
			...
			if (hasCollapsedSelection) {
				...
				if (probeIndex >= 0 && probeIndex < message.length) {
					...
					if (looksLikeFileMention) {
						...
						const removeUntil =
							message[tokenEnd] === " " ? tokenEnd + 1 : tokenEnd;
		...
		const cycleAgentBackwardShortcut =
			cycleAgentShortcut && !cycleAgentShortcut.includes("shift")
				? normalizeCombo(`shift+${cycleAgentShortcut}`)
				: "";
		const cycleAgentDirection =
			cycleAgentBackwardShortcut &&
			eventMatchesShortcut(e, cycleAgentBackwardShortcut)
				? -1
		...
		const cursorAtEnd =
			...
			textareaRef.current?.selectionEnd === message.length;
		const canNavigateHistoryUp =
			!isAnyAutocompleteOpen && (message.length === 0 || cursorAtStart);
		const canNavigateHistoryDown =
			!isAnyAutocompleteOpen && (message.length === 0 || cursorAtEnd);
	...
	const measureCaretInTextarea = React.useCallback(
		(textarea: HTMLTextAreaElement, cursorPosition: number) => {
			...
			marker.textContent =
				textarea.value.slice(cursorPosition, cursorPosition + 1) || " ";
	...
	const updateAutocompleteOverlayPosition = React.useCallback(() => {
		...
		const caretY =
			textareaRect.top - containerRect.top + (caret.top - textarea.scrollTop);
		const caretX =
			textareaRect.left -
			containerRect.left +
			(caret.left - textarea.scrollLeft);
		...
		const place: "above" | "below" =
			spaceBelow >= estimatedPopupHeight || spaceBelow >= spaceAbove
				? "below"
				: "above";

		const desiredWidth = showFileMention
			? 520
		...
		const maxHeight = Math.max(
			...
			Math.min(
				...
				place === "below" ? spaceBelow : spaceAbove,
			),
		);
	...
	const adjustTextareaHeight = React.useCallback(
		(options?: { allowShrink?: boolean }) => {
			...
			const paddingTotal =
				Number.isNaN(paddingTop) || Number.isNaN(paddingBottom)
					? fallbackPadding
					: paddingTop + paddingBottom;
			const targetLineHeight = Number.isNaN(lineHeight)
				? fallbackLineHeight
				: lineHeight;
			const maxHeight =
				targetLineHeight * MAX_VISIBLE_TEXTAREA_LINES + paddingTotal;
	...
	const updateAutocompleteState = React.useCallback(
		(value: string, cursorPosition: number) => {
			...
			if (lastSlashSymbol !== -1) {
				const charBefore =
					lastSlashSymbol > 0 ? textBeforeCursor[lastSlashSymbol - 1] : null;
			...
			if (lastHashSymbol !== -1) {
				const charBefore =
					lastHashSymbol > 0 ? textBeforeCursor[lastHashSymbol - 1] : null;
	...
	const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
		...
		if (isVSCodeRuntime() && suppressNextFileDropTextInsertRef.current) {
			...
			const isLikelyDropTextInsertion =
				nativeInputEvent?.inputType === "insertFromDrop" ||
				candidateAbsolutePaths.some(
					(path) => path.length > 0 && e.target.value.includes(path),
				);
	...
	const handlePaste = React.useCallback(
		async (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
			...
			for (let index = 0; index < imageFiles.length; index += 1) {
				...
				const file = renameFileForAttachmentCitation(
					imageFiles[index],
					filename,
				);
				...
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.clipboardAttachFailed"),
					);
				} finally {
	...
	const handleCommandSelect = (command: CommandInfo) => {
		...
		const textareaElement = textareaRef.current as HTMLTextAreaElement & {
			_commandMetadata?: typeof command;
		};
		if (textareaElement) {
	...
	const hasDraggedFiles = React.useCallback(
		(dataTransfer: DataTransfer | null | undefined): boolean => {
			...
			if (dataTransfer.types) {
				...
				if (lowerTypes.includes("application/x-openchamber-file-path"))
					return true;
				if (lowerTypes.some((type) => type.includes("vnd.code.tree")))
					return true;
			}
	...
	const collectDroppedFiles = React.useCallback(
		(dataTransfer: DataTransfer | null | undefined): File[] => {
	...
	const collectDroppedFileUris = React.useCallback(
	...
	const toProjectRelativeMentionPath = React.useCallback(
		(absolutePath: string): string => {
			...
			const normalizedRoot = (chatSearchDirectory || "")
				.replace(/\\/g, "/")
				.replace(/\/+$/, "");
			if (!normalizedRoot) {
	...
	const addVSCodeDroppedUrisAsMentions = React.useCallback(
		(uris: string[]) => {
			...
			toast.success(
				t("chat.chatInput.toast.addedFileMentions", { count: mentions.length }),
			);
		},
		[
	...
	const handleDragEnter = (e: React.DragEvent) => {
		...
		const isInternal =
			e.dataTransfer.types?.includes("application/x-openchamber-file-path") ??
			false;
	...
	const handleDrop = async (e: React.DragEvent) => {
		...
		const internalPath = e.dataTransfer.getData(
			"application/x-openchamber-file-path",
		);
			...
			for (const file of files) {
				try {
					...
					toast.error(
						error instanceof Error
							? error.message
							: t("chat.chatInput.toast.attachFileFailed"),
					);
				}
			}
		}
	...
	const handleVSCodePickFiles = React.useCallback(async () => {
		try {
			...
			toast.error(
				error instanceof Error
					? error.message
					: t("chat.chatInput.toast.vscodePickFailed"),
			);
		}
	...
	const selectedDraftProject = React.useMemo(() => {
		const explicit = newSessionDraft?.selectedProjectId
			? (projects.find(
					(project) => project.id === newSessionDraft.selectedProjectId,
				) ?? null)
			: null;
		if (explicit) {
			return explicit;
	...
	const [isDiscoveringDraftBranches, setIsDiscoveringDraftBranches] =
		React.useState(false);
	...
	React.useEffect(() => {
		if (
			...
			!runtimeGit ||
			selectedDraftProjectIsGitRepo !== null
		) {
			return;
		}
	...
	React.useEffect(() => {
		if (
			...
			!runtimeGit ||
			selectedDraftProjectIsGitRepo !== true
		) {
		...
		void fetchBranches(selectedDraftProjectPath, runtimeGit).finally(() => {
			if (!cancelled) {
				setIsDiscoveringDraftBranches(false);
			}
		});
	...
	const worktreeBranchOptions = React.useMemo(() => {
		...
		const worktrees = (() => {
			...
			return (
				availableWorktreesByProject.get(selectedDraftProjectPath) ??
		...
		return buildSessionTargetOptions({
			...
			worktrees,
			pendingBootstrapDirectory:
				newSessionDraft?.bootstrapPendingDirectory ?? null,
	...
	const handleBranchSelectorOpenChange = React.useCallback(
		async (open: boolean) => {
	...
	const shouldKeepMissingSelectedDraftDirectory = React.useMemo(() => {
		const pendingDirectory = normalizePath(
			newSessionDraft?.bootstrapPendingDirectory ?? null,
		);
		return Boolean(
	...
	const draftBranchItems = React.useMemo(() => {
		...
		return [
			...baseItems,
			{
				value: selectedDraftDirectory,
				label: formatDirectoryName(selectedDraftDirectory),
			},
		];
	}, [
		projectRootBranchOption,
		selectedDraftDirectory,
		shouldKeepMissingSelectedDraftDirectory,
		worktreeBranchOptions,
	]);

	const selectedDraftBranchLabel = React.useMemo(() => {
		const selectedValue =
			selectedDraftDirectory ?? draftBranchItems[0]?.value ?? null;
		if (!selectedValue) {
			return null;
		}
		return (
			draftBranchItems.find((item) => item.value === selectedValue)?.label ??
	...
	const selectedDraftBranchIsKnown = React.useMemo(() => {
		...
		return worktreeBranchOptions.some(
			(option) => option.value === selectedDraftDirectory,
		);
	}, [
		projectRootBranchOption?.value,
		selectedDraftDirectory,
		worktreeBranchOptions,
	]);
	...
	const shouldShowDraftBranchSelector = React.useMemo(() => {
		...
		projectRootBranchOption,
		selectedDraftProjectIsGitRepo,
		worktreeBranchOptions.length,
	]);

	const handleDraftProjectChange = React.useCallback(
		(projectId: string) => {
	...
	const handleDraftDirectoryChange = React.useCallback(
		(directory: string) => {
			const draft = useSessionUIStore.getState().newSessionDraft;
			if (
				draft?.pendingWorktreeRequestId ||
	...
	React.useEffect(() => {
		...
		const valid = draftBranchItems.some(
			(option) => option.value === selectedDraftDirectory,
		);
		if (valid) {
			return;
	...
	const buttonSizeClass = isMobile
		...
		: isVSCode
			? "h-5 w-5"
			: "h-6 w-6";
	const sendIconSizeClass = isMobile
		? "h-4 w-4"
		: isVSCode
			? "h-3.5 w-3.5"
			: "h-4 w-4";
	const stopIconSizeClass = isMobile
		? "h-6 w-6"
		: isVSCode
			? "h-4 w-4"
			: "h-5 w-5";
	...
	const permissionScopeSessionId =
		currentSessionId ?? currentManagementSessionId;
	...
	React.useEffect(() => {
		const pendingAbortBanner =
			Boolean(abortPromptSessionId) &&
			...
			>
				...
				!isMiniChatSurface ? (
					<div className="chat-input-column mb-7 text-center">
						<h1 className="text-balance text-2xl font-normal tracking-tight text-foreground md:text-3xl">
							{renderDraftTitle(
								draftProjectLabel
									? t("chat.emptyState.draftTitleWithProject", {
											project: draftProjectLabel,
										})
				...
				<div
					className={cn(
						...
						isDesktopExpanded && "flex flex-1 min-h-0 flex-col",
					)}
				>
							...
							<div className="flex w-full items-center gap-1.5 text-sm h-5 px-1">
								...
								>
									...
									<span className="text-muted-foreground flex-shrink-0">
										...
										{linkedIssue.author && (
											<span className="ml-1">
												{t("chat.chatInput.linked.byAuthor", {
													author: linkedIssue.author.login,
												})}
											</span>
										)}
									</span>
								...
								<span className="flex items-center gap-0.5 flex-shrink-0">
									<a
										...
										aria-label={t(
											"chat.chatInput.linked.issue.openInBrowserAria",
										)}
									>
										<Icon
											name="external-link"
						...
						<div className="pb-2 w-full px-1">
							<div className="flex w-full items-center gap-1.5 text-sm h-5 px-1">
								...
								>
									...
									<span className="text-muted-foreground flex-shrink-0">
										{t("chat.chatInput.linked.pr.number", {
											number: linkedPr.number,
										})}
										{linkedPr.author && (
											<span className="ml-1">
												{t("chat.chatInput.linked.byAuthor", {
													author: linkedPr.author.login,
												})}
											</span>
										)}
									</span>
							...
							>
								...
								<SelectContent fitContent>
									{projects.map((project) => (
										...
										>
											{renderProjectLabelWithIcon(project, currentTheme)}
										</SelectItem>
									))}
								</SelectContent>
							</Select>
									...
									<SelectContent className="w-max min-w-48">
										{projectRootBranchOption ? (
											<SelectGroup>
												...
												<SelectItem
													key={projectRootBranchOption.value}
													value={projectRootBranchOption.value}
													className="max-w-[24rem] truncate"
												>
					...
					>
								...
								<div className="text-center">
									<div className="inline-flex justify-center">
										...
										>
											<Icon
												name="attachment-2"
												className={cn(iconSizeClass, "text-current")}
											/>
										</button>
									</div>
									<p className="mt-2 typography-ui-label text-muted-foreground">
										{isInternalDrag
											? t("chat.chatInput.drop.insertMention")
											: t("chat.chatInput.drop.attachFiles")}
									</p>
								</div>
							</div>
						)}
						...
						{showCommandAutocomplete && (
							<CommandAutocomplete
								...
								style={
									isDesktopExpanded && autocompleteOverlayPosition
										? {
												...
												transform:
													autocompleteOverlayPosition.place === "above"
														? "translateY(-100%)"
														: undefined,
											}
										: undefined
							...
							>
								...
								<Textarea
									...
									className={cn(
										...
										highlightedComposerContent &&
											"text-transparent caret-[var(--surface-foreground)]",
									)}
									style={{
										flex: isDesktopExpanded ? "1 1 auto" : "none",
										height:
											!isDesktopExpanded && textareaSize
												? `${textareaSize.height}px`
												: undefined,
										maxHeight:
											!isDesktopExpanded && textareaSize
												? `${textareaSize.maxHeight}px`
												: undefined,
