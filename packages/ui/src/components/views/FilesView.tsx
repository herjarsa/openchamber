...
export const FilesView: React.FC<FilesViewProps> = ({ mode = "full" }) => {
	...
	const { currentTheme, availableThemes, lightThemeId, darkThemeId } =
		useThemeSystem();
	...
	const [showMobilePageContent, setShowMobilePageContent] =
		React.useState(false);
	const [wrapLines, setWrapLines] = React.useState(true);
	const [isFullscreen, setIsFullscreen] = React.useState(false);
	const [isSearchOpen, setIsSearchOpen] = React.useState(false);
	const [isFloatingToolbarOpen, setIsFloatingToolbarOpen] =
		React.useState(false);
	...
	const isClickInsidePortalledMenu = React.useCallback(
	...
	const [jsonViewMode, setJsonViewMode] = React.useState<"tree" | "text">(
		"tree",
	);
	const [htmlViewMode, setHtmlViewMode] =
		React.useState<PreviewViewMode>("edit");
	const [drawioViewMode, setDrawioViewMode] =
		React.useState<PreviewViewMode>("preview");
	...
	const openPaths = useFilesViewTabsStore((state) =>
		root ? (state.byRoot[root]?.openPaths ?? EMPTY_PATHS) : EMPTY_PATHS,
	);
	const selectedPath = useFilesViewTabsStore((state) =>
		root ? (state.byRoot[root]?.selectedPath ?? null) : null,
	);
	const expandedPaths = useFilesViewTabsStore((state) =>
		root ? (state.byRoot[root]?.expandedPaths ?? EMPTY_PATHS) : EMPTY_PATHS,
	);
	const addOpenPath = useFilesViewTabsStore((state) => state.addOpenPath);
	const removeOpenPath = useFilesViewTabsStore((state) => state.removeOpenPath);
	const removeOpenPathsByPrefix = useFilesViewTabsStore(
		(state) => state.removeOpenPathsByPrefix,
	);
	const setSelectedPath = useFilesViewTabsStore(
		(state) => state.setSelectedPath,
	);
	const toggleExpandedPath = useFilesViewTabsStore(
		(state) => state.toggleExpandedPath,
	);
	...
	const openFiles = React.useMemo(
		() => openPaths.map(toFileNode),
		[openPaths, toFileNode],
	);
	const effectiveSelectedPath = React.useMemo(
		() => selectedPath ?? openPaths[0] ?? null,
		[openPaths, selectedPath],
	);
	const selectedFile = React.useMemo(
		() => (effectiveSelectedPath ? toFileNode(effectiveSelectedPath) : null),
		[effectiveSelectedPath, toFileNode],
	);
	const selectedFilePath = selectedFile?.path ?? "";
	const selectedFileIsOutsideWorkspace = Boolean(
		root && selectedFilePath && !isPathWithinRoot(selectedFilePath, root),
	);
	const selectedFileReadOptions = React.useMemo(
		() => ({
			allowOutsideWorkspace:
				mode === "editor-only" && selectedFileIsOutsideWorkspace,
		}),
	...
	const [editorTabsOverflow, setEditorTabsOverflow] = React.useState<{
		left: boolean;
		right: boolean;
	...
	const activeDirectoryLoadIdsRef = React.useRef<Map<string, number>>(
		new Map(),
	);
	...
	const [imageAssetAuthReadyKey, setImageAssetAuthReadyKey] =
		React.useState("");

	const [loadedFilePath, setLoadedFilePath] = React.useState<string | null>(
		null,
	);

	const [draftContent, setDraftContent] = React.useState("");
	const [isSaving, setIsSaving] = React.useState(false);
	const [loadedFileLineEnding, setLoadedFileLineEnding] =
		React.useState<FileLineEnding>("\n");
	...
	const diagramAutoSaveTimerRef = React.useRef<ReturnType<
		typeof setTimeout
	> | null>(null);
	...
	const diagramEditorRef =
		React.useRef<React.ComponentRef<typeof DiagramEditor>>(null);
	const lastLoadedFileStatRef = React.useRef<FileStatSnapshot | null>(null);
	const activeFileLoadIdRef = React.useRef(0);
	const [autoSaveStatus, setAutoSaveStatus] = React.useState<"idle" | "saved">(
		"idle",
	);
	...
	const pendingTabRef = React.useRef<
		import("@/stores/useUIStore").MainTab | null
	>(null);
	...
	const pendingNavigationCycleRef = React.useRef<{
		key: string;
		attempts: number;
	}>({ key: "", attempts: 0 });
	...
	const [activeDialog, setActiveDialog] = React.useState<
		"createFile" | "createFolder" | "rename" | "delete" | null
	>(null);
	...
	const [contextMenuPath, setContextMenuPath] = React.useState<string | null>(
		null,
	);
	const [rightClickMenuPath, setRightClickMenuPath] = React.useState<
		string | null
	>(null);
	...
	const handleRevealPath = React.useCallback(
		(targetPath: string) => {
	...
	const handleOpenInApp = React.useCallback(
		async (app: { id: string; appName: string }) => {
			if (!selectedFile?.path) {
				return;
			}

			const openedInApp = await openDesktopFileInApp(
				selectedFile.path,
				app.id,
				app.appName,
			);
			...
			if (fileDirectory) {
				const openedDirectory = await openDesktopPath(
					fileDirectory,
					app.appName,
				);
				if (openedDirectory) {
	...
	const [lineSelection, setLineSelection] =
		React.useState<SelectedLineRange | null>(null);
	...
	const pendingFileNavigation = useUIStore(
		(state) => state.pendingFileNavigation,
	);
	const setPendingFileNavigation = useUIStore(
		(state) => state.setPendingFileNavigation,
	);
	const pendingFileFocusPath = useUIStore(
		(state) => state.pendingFileFocusPath,
	);
	const setPendingFileFocusPath = useUIStore(
		(state) => state.setPendingFileFocusPath,
	);
	const shortcutOverrides = useUIStore((state) => state.shortcutOverrides);
	const fileEditorKeymap = useUIStore((state) => state.fileEditorKeymap);
	const settingsDefaultFileViewerPreview = useConfigStore(
		(state) => state.settingsDefaultFileViewerPreview,
	);
	const showMessageTTSButtons = useConfigStore(
		(state) => state.showMessageTTSButtons,
	);
	...
	const extractSelectedCode = React.useCallback(
		(content: string, range: SelectedLineRange): string => {
	...
	const fileCommentController = useInlineCommentController<SelectedLineRange>({
		...
		language: selectedFile?.path
			? getLanguageFromExtension(selectedFile.path) || "text"
			: "text",
		getCodeForRange: (range) =>
			extractSelectedCode(fileContent, normalizeLineRange(range)),
	...
	const loadDirectory = React.useCallback(
		async (dirPath: string) => {
			...
			if (
				...
				inFlightDirsRef.current.has(normalizedDir)
			) {
				return;
			}
			...
			activeDirectoryLoadIdsRef.current = new Map(
				activeDirectoryLoadIdsRef.current,
			);
			activeDirectoryLoadIdsRef.current.set(normalizedDir, requestId);

			const isCurrentRequest = () =>
				activeDirectoryLoadIdsRef.current.get(normalizedDir) === requestId;

			const listPromise = files.listDirectory
				? files.listDirectory(normalizedDir).then((result) =>
				...
				: opencodeClient.listLocalDirectory(normalizedDir).then((result) =>
						result.map((entry) => ({
							name: entry.name,
							path: entry.path,
							isDirectory: entry.isDirectory,
				...
				await files
					...
					.then(async (result) => {
						if (result.success) {
							...
							if (
								selectedFile?.path === oldPath ||
								selectedFile?.path.startsWith(`${oldPath}/`)
							) {
								if (root) {
				...
				await files
					...
					.then(async (result) => {
						if (result.success) {
							...
							if (
								selectedFile?.path === deletedPath ||
								selectedFile?.path.startsWith(`${deletedPath}/`)
							) {
								if (root) {
									setSelectedPath(root, null);
								}
		...
		searchFiles(currentDirectory, trimmedQuery, 150, {
			...
			.then((hits) => {
				...
				const filtered = hits.filter(
					(hit) => showGitignored || !shouldIgnorePath(hit.path),
				);
	...
	const readFile = React.useCallback(
		async (
			path: string,
			options?: { allowOutsideWorkspace?: boolean; optional?: boolean },
		): Promise<string> => {
			if (files.readFile) {
	...
	const readFileStat = React.useCallback(
		async (
			path: string,
	...
	const saveDraft = React.useCallback(async () => {
		...
		if (
			draftContent === "" &&
			fileContent !== "" &&
			loadedFilePath !== selectedFile.path
		) {
			console.warn(
		...
		try {
			const contentToWrite = serializeEditorContent(
				draftContent,
	...
	const loadSelectedFile = React.useCallback(
		async (node: FileNode) => {
			...
			const isCurrentLoad = () => {
				...
				const currentPath =
					rootState?.selectedPath ?? rootState?.openPaths[0] ?? null;
				return (
					activeFileLoadIdRef.current === loadId && currentPath === node.path
				);
			};
			...
			const readOptions = {
				allowOutsideWorkspace:
					mode === "editor-only" &&
					Boolean(root) &&
					!isPathWithinRoot(node.path, root),
			};
			...
			await readFile(node.path, readOptions)
				.then((content) => {
					...
					setDraftContent(
						editorContent.length > MAX_VIEW_CHARS
	...
	const ensurePathVisible = React.useCallback(
		async (targetPath: string, includeTarget: boolean) => {
			if (!root) {
				return;
			}

			const ancestors = getAncestorPaths(targetPath, root);
			const pathsToExpand = includeTarget
				? [...ancestors, targetPath]
				: ancestors;
			...
			const loadPromises = pathsToExpand
				.map((path) => {
					if (!loadedDirsRef.current.has(path)) {
	...
	const getNextOpenFile = React.useCallback(
		(path: string, filesList: FileNode[]) => {
	...
	const getFileStatus = React.useCallback(
		(path: string): FileStatus | null => {
			// Check open status
	...
	const getFolderBadge = React.useCallback(
		(dirPath: string): { modified: number; added: number } | null => {
			...
			const relativeDir = dirPath.startsWith(root + "/")
	...
	const isSelectedImage = Boolean(
		selectedFile?.path && isImageFile(selectedFile.path),
	);
	const isSelectedSvg = Boolean(
		selectedFile?.path && selectedFile.path.toLowerCase().endsWith(".svg"),
	);
	...
	const isMarkdown = Boolean(
		selectedFile?.path && isMarkdownFile(selectedFile.path),
	);
	const isJson = Boolean(selectedFile?.path && isJsonFile(selectedFile.path));
	const isHtml = Boolean(selectedFile?.path && isHtmlFile(selectedFile.path));
	const isDrawio = Boolean(
		selectedFile?.path && isDrawioFile(selectedFile.path),
	);
	const isTextFile = Boolean(selectedFile && !isSelectedImage);
	const canUseShikiFileView =
		isTextFile &&
		!isMarkdown &&
		!isDrawio &&
	...
	const [dynamicLanguageExtension, setDynamicLanguageExtension] =
	...
	React.useEffect(() => {
		...
		const defaultMode: TextViewMode = settingsDefaultFileViewerPreview
			? "view"
			: "edit";
		...
		let mdDefault: PreviewViewMode = settingsDefaultFileViewerPreview
			? "preview"
			: "edit";
		try {
		...
		let htmlDefault: PreviewViewMode = settingsDefaultFileViewerPreview
			? "preview"
			: "edit";
		try {
		...
		setDrawioViewMode(
			drawioViewModeByPathRef.current[selectedPath] ?? "preview",
		);

		let jsonDefault: "tree" | "text" = settingsDefaultFileViewerPreview
			? "tree"
			: "text";
		try {
	...
	const saveTextViewMode = React.useCallback(
		(mode: TextViewMode) => {
			const selectedPath = selectedFile?.path;
			if (selectedPath) {
	...
	const saveMdViewMode = React.useCallback(
	...
	const saveHtmlViewMode = React.useCallback(
		(mode: PreviewViewMode) => {
			const selectedPath = selectedFile?.path;
			if (selectedPath) {
	...
	const saveDrawioViewMode = React.useCallback(
		(mode: PreviewViewMode) => {
			const selectedPath = selectedFile?.path;
			if (selectedPath) {
	...
	const saveDiagramXml = React.useCallback(
		async (path: string, xml: string) => {
			...
			setDraftContent(xml);
			const stat = await readFileStat(path, selectedFileReadOptions).catch(
				() => null,
			);
			if (stat) {
				lastLoadedFileStatRef.current = stat;
	...
	const handleDiagramChange = React.useCallback(
		(xml: string) => {
			diagramXmlRef.current = xml;
			if (
				!selectedFile?.path ||
	...
	React.useEffect(() => {
		...
		window.addEventListener(
			"openchamber:file-viewer-preview-mode-changed",
			handleFileViewerModeChanged,
		);
		return () => {
	...
	React.useEffect(() => {
		...
		const targetLineNumber = Math.max(
			1,
			Math.min(pendingFileNavigation.line, view.state.doc.lines),
		);
		...
		const shouldDispatch =
			!isAtTarget || pendingNavigationCycleRef.current.attempts === 0;
	...
	const nudgeEditorSelectionAboveKeyboard = React.useCallback(
	...
	React.useEffect(() => {
		...
		const goToLineCombo = getEffectiveShortcutCombo(
			"open_go_to_line",
			shortcutOverrides,
		);
	...
	const editorExtensions = React.useMemo(() => {
		...
		currentTheme,
		selectedFile?.path,
		staticLanguageExtension,
		dynamicLanguageExtension,
		wrapLines,
		isMobile,
	...
	const isImageAssetAuthLoading = Boolean(
		imageAssetAuthKey && imageAssetAuthReadyKey !== imageAssetAuthKey,
	);

	const imageSrc =
		selectedFile?.path && isSelectedImage
			? runtime.isDesktop
				? isSelectedSvg
				...
				: isSelectedSvg
					? `data:${getImageMimeType(selectedFile.path)};utf8,${encodeURIComponent(fileContent)}`
					: imageAssetAuthReadyKey === imageAssetAuthKey
	...
	React.useEffect(() => {
		...
		const resolveDesktopImage = async () => {
			...
			const srcPromise = files.readFileBinary
				? files
						.readFileBinary(selectedFile.path, selectedFileReadOptions)
	...
	const renderShikiFileView = React.useCallback(
		...
		return (
			<div className="pointer-events-auto flex items-center gap-1 rounded-lg border border-[var(--interactive-border)] bg-[var(--surface-elevated)] p-1 shadow-sm">
					...
					<>
						{isSaving ? (
							...
							withTooltip(
								t(
									autoSaveEnabled
										? "filesView.editor.saveNowTitle"
										: "filesView.editor.saveNowManualTitle",
									{ shortcut: `${getModifierLabel()}+S` },
								),
								<Button
									variant="ghost"
									size="sm"
									...
									aria-label={t("filesView.editor.saveAria", {
										shortcut: `${getModifierLabel()}+S`,
									})}
								>
						...
						{withTooltip(
							autoSaveEnabled
								? t("filesView.editor.autoSaveOn")
								: t("filesView.editor.manualSave"),
							<Button
								variant="ghost"
								size="sm"
					...
					<PreviewToggleButton
						...
						onToggle={() => {
							if (isHtmlFile(selectedFile?.path ?? "")) {
								saveHtmlViewMode(
									getHtmlViewMode() === "preview" ? "edit" : "preview",
								);
							} else {
								saveMdViewMode(
									getMdViewMode() === "preview" ? "edit" : "preview",
								);
							}
						}}
					/>
						...
						<Tooltip>
							<TooltipTrigger asChild>
								<Button
									...
									aria-label={
										isTTSPlaying
											? t("filesView.tts.stopSpeaking")
											: t("filesView.tts.readAloud")
									}
									onClick={() => {
										if (isTTSPlaying) {
											stopTTS();
								...
								>
									{isTTSPlaying ? (
										<Icon
											name="stop"
											className="size-4 text-[color:var(--status-success)]"
										/>
									) : (
				...
				{isDrawio && (
					<>
						<PreviewToggleButton
							...
							onToggle={() =>
								saveDrawioViewMode(
									drawioViewMode === "preview" ? "edit" : "preview",
								)
							}
						/>
						{drawioViewMode === "preview" && (
				...
				{isJson &&
					withTooltip(
						jsonViewMode === "tree"
							? t("filesView.editor.switchToTextView")
							: t("filesView.editor.switchToTreeView"),
						<Button
							variant="ghost"
							size="sm"
							onClick={() =>
								saveJsonViewMode(jsonViewMode === "tree" ? "text" : "tree")
							}
							className="size-6 p-0 text-muted-foreground opacity-65 hover:bg-transparent hover:opacity-100 focus-visible:bg-transparent active:bg-transparent"
							title={
								jsonViewMode === "tree"
				...
				{canCopy &&
					withTooltip(
						t("filesView.editor.copyFileContents"),
						<Button
							variant="ghost"
							size="sm"
							onClick={async () => {
				...
				{canCopyPath &&
					withTooltip(
						t("filesView.editor.copyFilePathTitle", {
						...
						<Button
							...
							className="size-6 p-0 hover:bg-transparent focus-visible:bg-transparent active:bg-transparent"
							title={t("filesView.editor.copyFilePathTitle", {
								path: displaySelectedPath,
							})}
							aria-label={t("filesView.editor.copyFilePathTitle", {
								path: displaySelectedPath,
							})}
						>
							{copiedPath ? (
	...
	// Floating controls slot content to pass to FilesPreview / FullscreenPreview
	const floatingSlot = selectedFile ? renderFloatingFileControls() : null;
	const floatingFullscreenSlot = selectedFile
		? renderFloatingFileControls({ exitFullscreenOnly: true })
		: null;
	const floatingTrigger = selectedFile ? (
		<Tooltip>
			<TooltipTrigger asChild>
				<span className="inline-flex">
					<Button
	...
	// Shared file content rendering for both normal and fullscreen preview
	const previewContent = !selectedFile ? (
		<div className="p-3 typography-ui text-muted-foreground">
		...
		<div className="p-3 typography-ui text-[color:var(--status-error)]">
			{fileError}
		</div>
	) : isSelectedImage ? (
		...
		<div className="h-full overflow-auto p-3">
			{fileContent.length > 500 * 1024 && (
				<div className="mb-3 rounded-md border border-status-warning/20 bg-status-warning/10 px-3 py-2 text-sm text-status-warning">
					{t("filesView.warning.largeFilePreviewLimited", {
						sizeKb: Math.round(fileContent.length / 1024),
					})}
				</div>
			)}
			<ErrorBoundary
		...
		<div className="h-full overflow-hidden">
			<iframe
				srcDoc={(() => {
					...
					const basePath = selectedFile.path.substring(
						0,
		...
		>
			<div
				className={cn(
					"h-full",
					shouldMaskEditorForPendingNavigation && "invisible",
				)}
			>
				<CodeMirrorEditor
					value={draftContent}
	...
	const treePanel = (
		...
		>
			<FilesToolbar
				...
				onNewFile={() =>
					handleOpenDialog("createFile", {
						path: currentDirectory,
						type: "directory",
					})
				}
				onNewFolder={() =>
					handleOpenDialog("createFolder", {
						path: currentDirectory,
						type: "directory",
					})
				}
	...
	const sharedPreviewProps = {
		...
		onHideMobileContent: () => setShowMobilePageContent(false),
		openFiles,
		selectedFile,
		onSelectFile: handleSelectFile,
		...
		alwaysShowActions,
		root,
		floatingControlsSlot: isMobile ? null : floatingSlot,
		floatingControlsTrigger: isMobile ? null : floatingTrigger,
	...
	const fullscreenPreview = (
		<FullscreenPreview
			...
			floatingControlsSlot={floatingFullscreenSlot}
		>
			{previewContent}
		</FullscreenPreview>
	);
	...
	return (
		<div className="flex h-full min-h-0 overflow-hidden bg-background relative">
			...
			{isMobile ? (
				showMobilePageContent ? (
					<FilesPreview {...sharedPreviewProps}>{previewContent}</FilesPreview>
				) : (
					treePanel
				)
			) : mode === "editor-only" ? (
				<div className="flex flex-1 min-h-0 min-w-0 overflow-hidden">
					<div className="flex-1 min-h-0 min-w-0 overflow-hidden bg-background">
						<FilesPreview {...sharedPreviewProps}>
							{previewContent}
						</FilesPreview>
					</div>
				</div>
