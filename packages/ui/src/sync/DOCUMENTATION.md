...
Subagent sessions (those with a `parentID`) route their `session.idle` and `session.error` events through `subagentNotificationBatcher` instead of the direct `appendNotification` path. The batcher coalesces sibling subagents that share the same parent into one consolidated notification per `(directory, parentID)` pair after a 1.2s debounce window. Per-session dedupe keeps retry cycles (error → idle for the same session) from inflating the batch. The notification body carries no user-facing text — the rendering component is expected to read the notification store's existing per-session/per-directory count indices and produce its own i18n strings. Top-level sessions still go through the direct notification path unchanged. The batcher is a process-wide singleton keyed by `${directory}:${parentID}`, so concurrent batches across directories and parents remain independent.
