...
- Sessions: subagent (background task) sessions now produce a single consolidated notification per parent instead of silently skipping subagent events altogether; previously a parent learned nothing when its subagents finished or failed (thanks to @herjarsa).
